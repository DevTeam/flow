namespace Flow.Core
{
    ...
    public static class Activities
    {
        ...
        public static IDictionary<string, object> Run(string activityName, TimeSpan timeout)
        {
            using (var container = Container.Create().Using<Configuration>())
            {
                ...
                var extensionKeys =
                    ...
                    where key.Type.Assembly != typeof(IContainer).Assembly && !key.Type.IsValueType && !key.Type.IsGenericTypeDefinition
                    select key;
